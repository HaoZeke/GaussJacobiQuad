...
Supported methods: ~rec~ / ~rec_caf~ (recurrence; ~rec_caf~ partitions Newton nodes across images),
~gw~ / ~algo665~ (Golub–Welsch / Algorithm 655 eigensolve). Multi-image speedup for
~gw~ and ~algo665~ uses independent *rules* via ~gauss_jacobi_batch_caf~ (serial
tridiagonal eigensolve per rule does not split across images).
