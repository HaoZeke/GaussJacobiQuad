...
Supported methods: ~rec~ (serial recurrence), ~rec_caf~ (Coarray-partitioned recurrence Newton nodes), ~gw~ (Golub–Welsch), ~algo665~.
...
#   cafrun -n 4 ./build/*/app/gjp_bench_caf 6144 0.5 0.5 7 3
# Timed driver ~gjp_bench_caf~ measures only the shipped ~rec_caf~ call.
# Example confirmed speedup (rg.terra, npts=6144, α=β=0.5, 7 trials):
#   1 image mean ≈ 2.01 s → 4 images mean ≈ 1.03 s (≈1.94×).
#
# Reproducible toolchain / CI image (Nix dockerTools): see ~docker/caf/~.
#   cd docker/caf && nix develop          # gfortran+MPICH+OpenCoarrays shell
