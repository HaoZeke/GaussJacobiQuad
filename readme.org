...
~rec~/~rec_caf~, ~gw~, ~algo665~/~algo665_dc~, ~sturm~/~sturm_caf~.
~imtqlx~ is *not* the absolute best for large ~n~; ~sturm_caf~ parallelizes the
same Golub-Welsch eigenproblem by eigenvalue index, ~rec_caf~ by Newton node.
