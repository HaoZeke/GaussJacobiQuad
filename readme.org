...
*** Releases
Releases are cut with [[https://docs.cocogitto.io/][cocogitto]] (~cog~) and [[https://towncrier.readthedocs.io/][towncrier]] (~NEWS.md~ from
~newsfragments/~). See [[file:docs/release.md][docs/release.md]] for the full checklist; in short, on ~main~:
...
PR commits are linted with ~cog check~ (GitHub Actions workflow ~commit-lint~).
Preferred commit types: ~feat~, ~fix~, ~docs~, ~chore~, ~ci~, ~build~, ~test~ (conventional commits).
* License
MIT.
*** Citation
