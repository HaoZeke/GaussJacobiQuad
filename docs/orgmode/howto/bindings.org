...
It multi-phase-inits (=PEP 489=), uses a heap =GaussJacobiError=, declares
=Py_MOD_GIL_NOT_USED= for free-threaded CPython 3.13+, and links the same
ISO_C_BINDING entry =gauss_jacobi_rule_c= into =_core.so= (no =GJP_CINTERP=).
